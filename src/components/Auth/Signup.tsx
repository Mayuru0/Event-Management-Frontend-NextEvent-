  ...
  return (
    <div className="min-h-screen bg-[#121212] p-2">
      ...
      <div className="mx-auto max-w-lg">
        <div className="rounded-lg bg-[#1F1F1F] border border-zinc-800 p-12 -mt-6">
  ...
  Join us today! Whether you&apos;re a Customer or an Organizer, we&apos;ve got you covered.
          ...
          <div className="flex justify-center mb-6 relative">
            <label
              ...
              className="relative w-24 h-24 rounded-full overflow-hidden bg-zinc-800 cursor-pointer"
            >
              {previewImage ? (
                <Image
                  src={previewImage}
              ...
              />
              {/* Show camera icon only when no image is selected */}
              {!previewImage && (
                <CiCamera className="absolute inset-0 m-auto text-white text-4xl" />
              )}
            </label>
            {errors.profileImage && (
              <p className="absolute mt-24 text-red-500 text-sm">
                {errors.profileImage}
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-gray-300 focus:outline-none focus:ring-2 focus:ring-violet-600 focus:border-transparent"
            >
            ...
            {inputFields.map((field) => (
              <div key={field.name}>
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-600 focus:border-transparent"
                  required
                />
                {errors[field.name] && (
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#6200EE] hover:bg-violet-700 text-white py-2 px-4 rounded-md transition-colors duration-200"
            >
